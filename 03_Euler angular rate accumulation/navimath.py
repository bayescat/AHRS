...
# Gyroscope 출력을 사용한 Euler angular rate 계산
def gyro_angular_rate (pre_attitude, 
                       Gyroscope):
    
    Euler_dot = np.zeros((3))
    
    phi, theta, psi = pre_attitude
    p, q, r = Gyroscope
    
    Euler_dot[0] = (p + q*(sin(phi) * tan(theta)) + r*(cos(phi) * tan(theta)))
    Euler_dot[1] =  (q*(cos(phi)) + r*(-sin(phi)))
    Euler_dot[2] = (q *(sin(phi) * 1/cos(theta)) + r*(cos(phi) * 1/cos(theta)))
